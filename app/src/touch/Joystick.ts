...
class BitmapBuilder {
  ...
  constructor(bmd:Phaser.BitmapData) {
    this.ctx = bmd.ctx;
  }
  ...
  circle(x:number, y:number, radius:number, borderColor?:string | CanvasGradient, width:number = 0, color?:string | CanvasGradient) {
    ...
    this.ctx.arc(0, 0, radius, 0, Phaser.Math.PI2);
